...
export class AdminLectorComponent implements OnInit {
  ...
  constructor(private route: ActivatedRoute,  private servicioEvento:EventoDataService, private lectorService: LectorDataService) { }
  ...
  manejar(){
    ...
    this.lectorService.getPalco(this.texto).subscribe((response)=>{
  ...
  recibirLectura(response){
    ...
    if(this.palco!=null){
      ...
      if(this.palco.vendido==true && this.palco.precioParcialPagado>=this.palco.servicio+this.palco.precio+this.palco.servicioIva){
...
if(!this.cargando){
  this.cargando=true
  this.lectorService.ingresarPersonas(this.palco.id,this.entrarPersonas).subscribe((response)=>{
